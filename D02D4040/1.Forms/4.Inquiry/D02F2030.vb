...
#Region "Const of tdbg - Total of Columns: 4"
    Private Const COL_OrderNum As String = "OrderNum"           ' STT
    Private Const COL_Description As String = "Description"     ' Thông tin
    Private Const COL_Total As String = "Total"                 ' Tổng cộng
    Private Const COL_FormatDisplay As String = "FormatDisplay" ' FormatDisplay
#End Region
    ...
    Private Sub D02F2030_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        'ID : 255329 - BỔ SUNG COMBO NHÓM THEO VÀ MÃ LOẠI PHÂN TÍCH
        oFilterCombo = New Lemon3.Controls.FilterCombo
        oFilterCombo.UseFilterCombo(tdbcTypeCodeID)
    ...
    Private Sub LoadLanguage()
        ...
        '================================================================ 
        lblTypeCodeID.Text = rL3("Ma_loai_PT_tai_san") 'Mã loại PT tài sản
        lblGroupID.Text = rL3("Nhom_theo") 'Nhóm theo
        '================================================================ 
        tdbcGroupID.Columns("GroupID").Caption = rL3("Ma") 'Mã
        tdbcGroupID.Columns("GroupName").Caption = rL3("Dien_giai") 'Diễn giải
    ...
    Private Sub LoadTDBCombo()
        ...
        'ID : 255329 - BỔ SUNG COMBO NHÓM THEO VÀ MÃ LOẠI PHÂN TÍCH
        sSQL = " --Do nguon cho combo Nhom theo " & vbCrLf
        sSQL &= " SELECT      0 AS GroupID, N'Không nhóm' AS GroupName" & vbCrLf
        sSQL &= " UNION ALL" & vbCrLf
        sSQL &= " SELECT      1 AS GroupID, N'Nhóm theo loại PT tài sản 'AS GroupName" & vbCrLf
        LoadDataSource(tdbcGroupID, sSQL, gbUnicode)

        sSQL = " --Do nguon cho combo Mã loại PT tài sản " & vbCrLf
        sSQL &= " SELECT TypeCodeID,VieTypeCodeNameU AS Description" & vbCrLf
        sSQL &= " FROM D02T0040 WITH(NOLOCK) " & vbCrLf
        sSQL &= " WHERE Type = 'A' AND Disabled = 0" & vbCrLf
...
#Region "Cau lenh SQL"
    ...
    Private Function SQLStoreD02P2030() As String
        ...
        sSQL &= SQLNumber(chkIsViewDetail.Checked) & COMMA 'IsViewDetail, tinyint, NOT NULL
        'ID : 255329 - BỔ SUNG COMBO NHÓM THEO VÀ MÃ LOẠI PHÂN TÍCH
        sSQL &= SQLNumber(ReturnValueC1Combo(tdbcGroupID)) & COMMA 'GroupID, varchar[20], NOT NULL 
        sSQL &= SQLString(ReturnValueC1Combo(tdbcTypeCodeID))  'TypeCodeID, varchar[20], NOT NULL
        Return sSQL
    End Function
...
#End Region
    ...
    Private Sub btnFilter_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnFilter.Click
        ...
        'LoadCaptionSplit2()
        Me.Cursor = Cursors.WaitCursor
        dtCol = ReturnDataTable(SQLStoreD02P2031)
        If dtFormat IsNot Nothing Then dtFormat.Clear()
        AddColFormatText(dtFormat, tdbg, "N" & Numberformat.ToString (), COL_Total) '118416  - 05 Mar 2019
    ...
    Private Sub AddColumns(ByVal i As Integer, ByVal dtCol As DataTable)
        ...
        Select Case L3String(dtCol.Rows(i).Item("DataType"))
            Case "N"
                ...
                AddColFormatText(dtFormat, tdbg, dtCol.Rows(i).Item("DataFormat").ToString, sField)
    ...
    Private Sub tdbg_FetchRowStyle(ByVal sender As Object, ByVal e As C1.Win.C1TrueDBGrid.FetchRowStyleEventArgs) Handles tdbg.FetchRowStyle
        ...
        ElseIf tdbg(e.Row, COL_FormatDisplay).ToString = "TITLE" Then 'ID : 255329 - BỔ SUNG COMBO NHÓM THEO VÀ MÃ LOẠI PHÂN TÍCH
            e.CellStyle.Font = FontUnicode(gbUnicode, FontStyle.Bold)
    ...
    Private Sub CallD09U1111_Button(ByVal bLoadFirst As Boolean)
        ...
        If bLoadFirst = True Then
            Dim arrColObligatory() As Integer = {IndexOfColumn(tdbg, COL_Description), IndexOfColumn(tdbg, COL_Total)}
    ...
    'ID : 255329 - BỔ SUNG COMBO NHÓM THEO VÀ MÃ LOẠI PHÂN TÍCH
    Private Sub tdbcTypeCodeID_Validated(sender As Object, e As EventArgs) Handles tdbcTypeCodeID.Validated
        oFilterCombo.FilterCombo(tdbcTypeCodeID, e)
    End Sub


    Private Sub tdbcGroupID_SelectedValueChanged(sender As Object, e As EventArgs) Handles tdbcGroupID.SelectedValueChanged
        If ReturnValueC1Combo(tdbcGroupID) = "1" Then
            tdbcTypeCodeID.Enabled = True
        Else
            tdbcTypeCodeID.Enabled = False
            If ReturnValueC1Combo(tdbcTypeCodeID) <> "" Then
                tdbcTypeCodeID.Text = ""
            End If
        End If
    End Sub

    Private Sub tdbcColData_SelectedValueChanged(sender As Object, e As EventArgs) Handles tdbcColData.SelectedValueChanged
        If ReturnValueC1Combo(tdbcColData) = "ACC" Then
            tdbcGroupID.Enabled = True
        Else
    ...
    Public Sub New()
        ...
        InitializeComponent()
        AnchorForControl(EnumAnchorStyles.TopLeftRight, False, grp1)
        AnchorResizeColumnsGrid(EnumAnchorStyles.TopLeftRightBottom, False, tdbg)
